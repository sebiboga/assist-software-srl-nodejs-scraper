...
const ASSIST_ANAF_RECORD = {
  ...
  legalForm: 'SRL',
  headquartersAddress: { locality: 'Suceava' },
  administrators: [{ name: 'ADMINISTRATOR NAME', role: 'administrator' }],
...
describe('company.js', () => {
  ...
  describe('getCompanyBrand', () => {
    it('should return the company brand', () => {
      ...
      expect(brand).toBe('ASSIST SOFTWARE');
    });
  });

  describe('getCompanyData (no cache)', () => {
    it('should find ASSIST via ANAF search and return company data', async () => {
      mockFetch
      ...
      expect(result).toHaveProperty('company', 'ASSIST SOFTWARE SRL');
      expect(result).toHaveProperty('cif', '2693736');
      expect(result).toHaveProperty('active', true);
      expect(result).toHaveProperty('anafData');
      expect(result.anafData.name).toBe('ASSIST SOFTWARE SRL');
    });
    ...
    it('should throw when no active company found', async () => {
      mockFetch.mockResolvedValueOnce(anafSearchResponse([
        { cui: 11111111, name: 'ASSIST SOMETHING SRL', statusLabel: 'Radiată' }
      ]));
    ...
    it('should throw when ANAF returns no company name', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(anafCompanyResponse({ cui: 2693736, name: null }));
  ...
  describe('validateAndGetCompany', () => {
    it('should return company data with status active', async () => {
      mockFetch
        ...
        .mockResolvedValueOnce(peviitorResponse([{ company: 'ASSIST SOFTWARE SRL' }]));
    ...
    it('should return inactive status when company is inactive', async () => {
      const inactiveRecord = { ...ASSIST_ANAF_RECORD, inactive: true };
